...
(function ($, window, undefined) {
	...
	var TutorialController = function (scrollAlways, closeOnEnd, 
                destroyTutorial) {
		var self = this;
		...
		var _scrollAlways = scrollAlways;
                ...
                // control close on end tutorial
                var _closeOnEnd = closeOnEnd;
                
                // 
                var _destroyTutorialCallback = destroyTutorial;
		...
		self.nextPrevStep = function (nextOrPrev) {
			...
			if (!_currStep) {
				...
				self.activateStep(_stepsIndexed[nextIndex]);
                                ...
                                if (_startWithStep.index == nextIndex && _closeOnEnd) {                            
                                    _destroyTutorialCallback.call(self);      
                                } 
			}
		};
	...
	Plugin.prototype.init = function () {
		...
		var tutorialController = new TutorialController(this.options.scrollAlways,
                    this.options.closeOnEnd, function () {
                        self.destroy();
                    });
		...
		$(document).on("keyup.osh", function (e) {
			...
			if (!self.options.allowEventPropagation) {
                ...
                if (self.options.autoPagination > 0) {
                    var autoPaginationEvent = setInterval(function() {
                        tutorialController.nextPrevStep(1);
                    }, 
                    self.options.autoPagination);
                }
	...
	Plugin.prototype.destroy = function () {
		...
		$(document).off(".osh");                
                window.clearInterval(this.autoPaginationEvent);
	}
