...
/**
 * Controlador del contenido de la tabla de destinatarios.
...
public class CJSonTable extends AbstractCDataTable implements Serializable {
    ...
    public String getJson() {
        List<Destinatario> aTable = destinatarioFacade.findAllFiltering(this, session.getAccessInfo());
        DTResponse<Destinatario> dtResponse = new DTResponse<Destinatario>(aTable.size(), aTable.size(),
    ...
    private DestinatarioFacadeLocal lookupDestinatarioFacadeLocal() {
        try {
            ...
            logger.error("Imposible obtener el bean de session Destinatario.");
